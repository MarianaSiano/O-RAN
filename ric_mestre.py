...
PORT = 8080
POLICY_ENERGY_SAVING = False # Estado global da política A1
...
class TrafficHandler(http.server.SimpleHTTPRequestHandler):
    def log_message(self, format, *args):
        return # Limpa logs de acesso do console

    def do_GET(self):
        # Simula Interface E2 (Downlink): Envia estado da rede
        global POLICY_ENERGY_SAVING

        # Se politica A1 ativa, reduz capacidade simulada
    ...
    def do_PUT(self):
        # Simula Interface A1: rApp envia Politica
        global POLICY_ENERGY_SAVING
    ...
    def do_POST(self):
        # Simula Interface E2 (Uplink): xApp envia Telemetria
        length = int(self.headers['Content-Length'])
        data = json.loads(self.rfile.read(length).decode())

        # Captura dados para o Benchmark Científico
...
print(f"--- Interface A1 (PUT) e E2 (GET/POST) Ativas ---")
